          ...
          <section className="bg-white border-b border-gray-200">
            <div className="max-w-4xl mx-auto px-4 py-16 md:py-24 text-center">
              ...
              {page.ctaLink && (
                <Link 
                  href={page.ctaLink}
                  className="inline-flex items-center justify-center gap-3 bg-[#003399] hover:bg-blue-800 text-white font-black text-lg py-4 px-10 rounded-full transition-all shadow-xl hover:-translate-y-1"
                >
          ...
          <div className="max-w-5xl mx-auto px-4 py-12 space-y-16">
            {embedUrl && (
              <section className="relative rounded-3xl overflow-hidden shadow-2xl bg-black aspect-video max-w-4xl mx-auto border-4 border-white">
                <iframe src={embedUrl} title="YouTube video player" allowFullScreen className="absolute inset-0 w-full h-full"></iframe>
              </section>
            )}
                ...
                <section className="grid grid-cols-2 gap-4">
                  {page.photos.map((photo, index) => (
                    <div key={index} className={`relative rounded-3xl overflow-hidden shadow-sm bg-gray-200 border-4 border-white ${index === 0 && page.photos.length % 2 !== 0 ? 'col-span-2 aspect-video' : 'aspect-square'}`}>
                      <Image src={photo} alt="Gallery" fill className="object-cover hover:scale-105 transition-transform duration-500" />
                    </div>
                  ))}
                </section>
              )}
